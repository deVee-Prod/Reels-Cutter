...
export default function ReelsCutterPage() {
  ...
  const processVideo = async () => {
    ...
    try {
      ...
      // 1. סריקת שתיקות מהירה (Audio Only)
      let logData = "";
      ...
      setStatus("Rendering (Optimization On)...");

      // 2. בניית הפילטר המנצח (720p + 30fps = מהירות)
      let filterComplex = '';
      let concatInputs = '';
      segments.forEach((seg, i) => {
        const start = seg.start.toFixed(3);
        const endOpt = seg.end !== null ? `:end=${seg.end.toFixed(3)}` : '';
        filterComplex += `[0:v]trim=start=${start}${endOpt},setpts=PTS-STARTPTS,fps=30,scale=720:-2[v${i}];`;
      ...
      a.href = url;
      a.download = `deVee_Cut_${videoFile.name}.mp4`;
      a.click();
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-between py-12 px-6 font-sans">
      ...
      <div className="flex flex-col items-center z-10 text-center">
        ...
        <p className="text-gray-600 text-[7px] tracking-[0.3em] mt-2 uppercase font-light">Unlimited Client Engine</p>
      </div>
      ...
      <div className="w-full max-w-[550px] bg-[#0c0c0c] border border-white/[0.03] rounded-[40px] p-10 shadow-2xl relative">
        ...
        <div className="relative flex flex-col items-center">
          <label className="w-full cursor-pointer group">
            <div className="border border-dashed border-white/[0.07] group-hover:border-[#D4AF37]/40 rounded-[30px] py-16 bg-white/[0.01] transition-all duration-500">
              <span className="text-[9px] uppercase tracking-[0.2em] text-gray-500 group-hover:text-gray-300">
                {videoFile ? videoFile.name : "Select Video (4K Supported)"}
              </span>
            </div>
          ...
          {processing && (
            <div className="w-full mt-8 px-2 text-center text-[10px] text-[#D4AF37]">
          ...
          <button 
            ...
            `}
          >
            {processing ? "Processing Locally..." : "Start Fast Cut"}
          </button>
        </div>
      </div>
